...
def time_state(t_control,seed_list,seed_num,meet,fc_prop,s_start,e_start):
	...
	if numorl==1:
		...
		plt.plot(t,e,label='Pu={}'.format(fc),color="#0072B2", linestyle="-")
	elif numorl==2:
		plt.plot(t,e,label='Pu={}'.format(fc),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e,label='Pu={}'.format(fc),color="#009E73",  linestyle="--")
...
for numorl in unmorm_list:
	time_state(600,seed_list,10000,0.3,numorl,0.05,0.26)

plt.xlabel('Time')  
plt.ylabel('Proportion of immune(%)')  
plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG16c.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
