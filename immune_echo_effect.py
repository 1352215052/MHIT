...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state2(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start,echo):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
...
# 1. 设置字体（在绘图之前设置）
	...
	seed_list=seed_create2(10000,0.05,0.26)
	time_state2(600,seed_list,10000,0.3,1,0.05,0.26,0.6)


	plt.xlabel("Time")
	plt.ylabel("Proportion of individuals(%)")
	#plt.title('With echo chamber',fontsize=12)  
	plt.legend(loc='upper right')
	plt.tight_layout()
	plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG8b.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
