...
#大跨度转换通道
def widerange_convert(seed_list,value,meet_other,vn):
	...
	if value<0 and seed_list[meet_other]<0:#（传播者--传播者）
		...
		value+=random.randint(1,vn)
		seed_list[meet_other]+=-random.randint(1,vn)
	#无知者
	elif (value>=0 and value<4) and seed_list[meet_other]<0:#（无知者--传播者）
...
#状态转化函数
def state_convert(seed_list,fc_prop,vn):
	# print(seed_list)
	...
	for index, value in enumerate(seed_list):  #
		...
		for  meet_other in meet_other_indices:
			...
			if index != meet_other: #如果不是自己则继续
				...
				if rand_num<0:
					seed_list=widerange_convert(seed_list,value,meet_other,vn)
				else:
...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,meet,fc_prop,s_start,e_start,vn):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
	for i in range(0,t_control):
		...
		if i ==0:  #第一次（初始值）不参与状态转换
			...
			seed_list_convert=state_convert(encounter_list,fc_prop,vn)
	...
	if vn==20:
		...
		plt.plot(t,e,label='Vu=[{},{}]'.format(0,vn),color="#0072B2", linestyle="-")
	elif vn==50:
		plt.plot(t,e,label='Vu=[{},{}]'.format(0,vn),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e,label='Vu=[{},{}]'.format(0,vn),color="#009E73",  linestyle="--")
...
meet_list=[20,50,80]
seed_list=seed_create(1000,0.05,0.26)
for vn in meet_list:
	time_state(600,seed_list,1000,0.3,1,0.05,0.26,vn)
 
plt.xlabel('Time')  
plt.ylabel('Proportion of immune(%)')  
plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG16d.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
