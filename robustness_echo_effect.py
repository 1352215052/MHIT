...
def plot_fig2(num,s_int,e_int,t_control,t,cr,pu,echo):

	# plt.figure(figsize=(5,4)) # 设置图形大小（宽，高），单位为英寸
	e_last=[]
	seed_list1=seed_create2(num,s_int,e_int)
	for i in  range(t_control):
		e_lst=time_state2(t,seed_list1,num,cr,pu,s_int,e_int,echo)
		# time_state2(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start,echo)
		#time_state2(600,seed_list,1000,0.3,1,0.05,0.26,0.6)
		e_last.append(e_lst) #统计百分比
		
	dt=1 #时间间隔
	t= np.arange(0,t_control, dt)  # 生成时间数组  
	plt.plot(t, e_last, color="#D55E00",  linestyle="-.",  label="With echo chamber")
	#plt.show()
