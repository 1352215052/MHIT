...
#常规转换通道
def norn_convert(seed_list,value,meet_other,sel):
	if value<0 and seed_list[meet_other]<0:#（传播者--传播者）
		value+=-sel
		seed_list[meet_other]+=-sel
	...
	#免疫者
		...
		value+=sel-0.03
		seed_list[meet_other]+=sel-0.03

	return seed_list


#大跨度转换通道
def widerange_convert(seed_list,value,meet_other,sel):
...
#状态转化函数
def state_convert(seed_list,fc_prop,sel):
	# print(seed_list)
	...
	for index, value in enumerate(seed_list):  #
		...
		for  meet_other in meet_other_indices:
			...
			if index != meet_other: #如果不是自己则继续
				...
				if rand_num<0:
					seed_list=widerange_convert(seed_list,value,meet_other,sel)
				else:
					seed_list=norn_convert(seed_list,value,meet_other,sel)
...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,num_prop,fc_prop,s_start,e_start,sel):
	...
	for i in range(0,t_control):
		...
		seed_list_convert=state_convert(encounter_list,fc_prop,sel)
...
def plotfigre(sel_list):
	...
	for sel in sel_list:
		...
		seed_list_attenuation=time_state(350,seed_list,0.3,1,0.05,0.26,sel)
	...
	dt=0.01 #时间间隔
	t= np.arange(sel_list[0], sel_list[-1]+0.01, dt)  # 生成时间数组  

	plt.plot(t,por_list, color="lightcoral", linestyle='-.', linewidth=2)
	plt.xlabel('Selective psychological intensity(SPI)')  
	plt.ylabel('Group attitude divergence(GAD)')  
	#plt.grid(True)  #是否显示网格线
	#plt.legend()  
	plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG7.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
...
sel_list=[round((i*0.01),2) for i in range(5,25)]
plotfigre(sel_list)
