...
def threshold_find(start_thred,end_thred,t_test,t_control,seed_num,num_prop,fc_prop,s_start):
	e_avg=[] #平均值列表
	e=start_thred  #初始阈值
	num=(end_thred-start_thred)*100/3
	...
	# 生成更多的点用于插值
	x_new = np.linspace(t.min(), t.max(), 100)  # 生成300个点

	# 使用三次样条插值
	spl = make_interp_spline(t, e_avg, k=3)  # k=3 表示三次插值
	y_smooth = spl(x_new)
	...
	plt.plot(x_new,	y_smooth,label='Believers(steady state)',color="#0072B2")

	plt.xlabel('Initial immune proportion(%)')  
	plt.ylabel('Equilibrium Believer proportion(%)') 
	plt.axhline(y=0.15, color="#9E0003", linestyle=':', linewidth=0.5, label='Herd immunity criterion')

	plt.legend()  
	#plt.grid(True)  #是否显示网格线
	plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG18b.tif',
				format="tif" ,
				dpi=1000,
				bbox_inches="tight",
...
threshold_find(0.24,0.42,10,400,10000,0.3,1,0.05)
