...
#常规转换通道
def norn_convert(seed_list,value,meet_other,d_s,e_s):
	if value<0 and seed_list[meet_other]<0:#（传播者--传播者）
		value+=-d_s
		seed_list[meet_other]+=-d_s
...
#大跨度转换通道
def widerange_convert(seed_list,value,meet_other,d_s,e_s):
...
#状态转化函数
def state_convert(seed_list,fc_prop,d_s,e_s):
	# print(seed_list)
	...
	for index, value in enumerate(seed_list):  #
		...
		for  meet_other in meet_other_indices:
			...
			if index != meet_other: #如果不是自己则继续
				...
				if rand_num<0:
					seed_list=widerange_convert(seed_list,value,meet_other,d_s,e_s)
				else:
					seed_list=norn_convert(seed_list,value,meet_other,d_s,e_s)
...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start,d_s,e_s):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
	for i in range(0,t_control):
		...
		if i ==0:  #第一次（初始值）不参与状态转换
			...
			seed_list_convert=state_convert(encounter_list,fc_prop,d_s,e_s)
	...
	if d_s==0.06:
		...
		plt.plot(t,e,label='Vb={},Vi={}'.format(d_s,e_s),color="#0072B2", linestyle="-")
	elif d_s==0.07:
		plt.plot(t,e,label='Vb={},Vi={}'.format(d_s,e_s),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e,label='Vb={},Vi={}'.format(d_s,e_s),color="#009E73",  linestyle="--")
...
thred_list=[[0.06,0.03],[0.07,0.04],[0.08,0.05]]
for num in thred_list:
	d_s=num[0]
	e_s=num[1]
	seed_list=seed_create(10000,0.05,0.28)
	time_state(600,seed_list,10000,0.3,1,0.05,0.28,d_s,e_s)

plt.xlabel('Time')  
plt.ylabel('Proportion of immune(%)')  
plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG15b.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
