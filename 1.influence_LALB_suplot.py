...
def seed_create(seed_num,s,e,num):
	...
	if len(postive_indices) < e_start:  
	    ...
	    for index in selected_postive_indices:  
	        seed_list[index] = random.randint(num,num+4)
	return seed_list
...
#常规转换通道
def norn_convert(seed_list,value,meet_other,num):
	if value<0 and seed_list[meet_other]<0:#（传播者--传播者）
		value+=-0.06
		seed_list[meet_other]+=-0.06
	elif value<0 and (seed_list[meet_other]>=0 and seed_list[meet_other]<num):#（传播者--无知者））
		value+=0
		seed_list[meet_other]+=-0.06
	elif value<0 and seed_list[meet_other]>=num:#（传播者--免疫者）
		value+=0.03
		seed_list[meet_other]+=-0.06
	#无知者
	elif (value>=0 and value<num) and seed_list[meet_other]<0:#（无知者--传播者）
		value+=-0.06
		seed_list[meet_other]+=0

	elif (value>=0 and value<num) and (seed_list[meet_other]>=0 and seed_list[meet_other]<num):#（无知者--无知者）
		value+=0
		seed_list[meet_other]+=0
	elif (value>=0 and value<num) and seed_list[meet_other]>=num:#（无知者--免疫者）
		value+=0.03
		seed_list[meet_other]+=0
	#免疫者
	elif value>=num and seed_list[meet_other]<0:#（免疫者--传播者）
		value+=-0.06
		seed_list[meet_other]+=0.03

	elif value>=num and (seed_list[meet_other]>=0 and seed_list[meet_other]<num):#（免疫者--无知者）
		value+=0
		seed_list[meet_other]+=0.03
	elif value>=num and seed_list[meet_other]>=num:#（免疫者--免疫者）
		value+=0.03
		seed_list[meet_other]+=0.03
...
#大跨度转换通道
def widerange_convert(seed_list,value,meet_other,num):
...
#状态转化函数
def state_convert(seed_list,fc_prop,num):
	# print(seed_list)
	...
	for index, value in enumerate(seed_list):  #
		...
		for  meet_other in meet_other_indices:
			...
			if index != meet_other: #如果不是自己则继续
				...
				if rand_num<0:
					seed_list=widerange_convert(seed_list,value,meet_other,num)
				else:
					seed_list=norn_convert(seed_list,value,meet_other,num)
...
def state_attenuation(seed_list_convert,s_start,e_start,num):
	#print(seed_list_convert)
	s_li=[i for i in seed_list_convert if i <0]
	i_li=[i for i in seed_list_convert if i >=0 and i <num]
	e_li=[i for i in seed_list_convert if i >=num]
...
#统计每种状态数量
def state_sum(seed_list_convert,seed_num,num):
...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start,num):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
	for i in range(0,t_control):
		...
		if i ==0:  #第一次（初始值）不参与状态转换
			s_port,i_port,e_port=state_sum(seed_list,seed_num,num)  #1.每种状态人数变化
			# anti=antibody_sum()
			s.append(s_port)
			w.append(i_port)
			e.append(e_port)
			...
			seed_list_convert=state_convert(encounter_list,fc_prop,num)
			seed_list=seed_list_convert+unencounter_list

			seed_list_attenuation=state_attenuation(seed_list,s_start,e_start,num)  #衰减函数处理
			s_port,i_port,e_port=state_sum(seed_list_attenuation,seed_num,num)
			s.append(s_port)
			w.append(i_port)
			e.append(e_port)
	...
	if num==4:
		...
		plt.plot(t,e,label='CR1={},CR2={}'.format(0,num),color="#0072B2", linestyle="-")
	elif num==6:
		plt.plot(t,e,label='CR1={},CR2={}'.format(0,num),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e,label='CR1={},CR2={}'. format(0,num),color="#009E73",  linestyle="--",linewidth=2)
...
num_list=[4,6,8]
for num in num_list:
	seed_list=seed_create(10000,0.05,0.28,num)
	time_state(600,seed_list,10000,0.3,1,0.05,0.28,num)

 
plt.xlabel('Time')  
plt.ylabel('Proportion of immune(%)')  
plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG15a.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
