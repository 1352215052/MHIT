...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
	...
	if num==2000:
		...
		plt.plot(t,e,label='N={}'.format(seed_num),color="#0072B2", linestyle="-")
	elif num==5000:
		plt.plot(t,e,label='N={}'.format(seed_num),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e,label='N={}'.format(seed_num),color="#009E73",  linestyle="--")
...
num_list=[2000,5000,10000]
for num in num_list:
	seed_list=seed_create(num,0.05,0.26)
	time_state(600,seed_list,num,0.3,1,0.05,0.26)
