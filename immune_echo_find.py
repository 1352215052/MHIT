...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e,echo):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e_list1=[]
	#控制时间（时步）
...
#稳定性检验函数
def stability_test(t_test,t_control,seed_num,e,num_prop,fc_prop,s_start,echo):
	e_last_list=[]
	for i in range(0,t_test):
		seed_list=seed_create(seed_num,s_start,e)
		e_last=time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e,echo)
...
#1、绘图函数(时间--各群体状态变化曲线）)
def threshold_find(start_thred,end_thred,t_test,t_control,seed_num,num_prop,fc_prop,s_start,echo):
	e_avg=[] #平均值列表
	e=start_thred  #初始阈值
	num=(end_thred-start_thred)*100
	#for i in range(int(num)+1):
	for i in range(int(num)+1):
		e_last_list=stability_test(t_test,t_control,seed_num,e,num_prop,fc_prop,s_start,echo)
	...
	plt.plot(t,e_avg,label='With echo chamber',color="#D55E00",  linestyle="-.")
