...
def state_bar(seed_list_find):
	...
	for seed_list in seed_list_find:
		#print(len(seed_list))
		if a==1:
			plt.hist(seed_list, label='Believer',color='#D55E00', alpha=0.7, bins=3)  
			...
			plt.hist(seed_list, label='Immune',color='#0072B2', alpha=0.7, bins=10)  
...
seed_list_attenuation=time_state(400,seed_list,0.3,1,0.05,0.26)
...
# text_content="GAD={}".format(por_val)


# #plt.grid(True)  #是否显示网格线
# plt.text(-1,1500,text_content)
...
# plt.title('(a) Without selective exposure', y=-0.45, pad=20)
...
#水平辅助线axhline()
plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG6a.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
