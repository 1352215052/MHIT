...
def count_index(e_last):
	...
	semall_index=1-big_index
	return big_index,semall_index

def plot_fig2(num,s_int,e_int,t_control,t,cr,pu):

	# plt.figure(figsize=(5,4)) # 设置图形大小（宽，高），单位为英寸
	e_last=[]
	seed_list1=seed_create2(num,s_int,e_int)
	for i in  range(t_control):
		e_lst=time_state2(t,seed_list1,num,cr,pu,s_int,e_int)
		e_last.append(e_lst) #统计百分比
		
	dt=1 #时间间隔
	t= np.arange(0,t_control, dt)  # 生成时间数组  
	plt.plot(t, e_last, color="#D55E00",  linestyle="-.", label="With selective exposure ")
	# plt.show()
