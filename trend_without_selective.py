...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state1(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e_start):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e=[]
	#控制时间（时步）
	...
	plt.plot(t, e, color='#0072B2',linestyle="-", label="Immune")
	plt.plot(t, s, color='#D55E00', linestyle="--", label="Believer")
	plt.plot(t, w, color='#009E73',linestyle="-.", label="Neutral")
	# plt.plot(t, e, linestyle="-", label="Immune")
	# plt.plot(t, s,  linestyle="--", label="Believer")
	# plt.plot(t, w, linestyle="-.", label="Neutral")
...
# 1. 设置字体（在绘图之前设置）
	#plt.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'DejaVu Sans','Times New Roman']
	...
	seed_list1=seed_create1(10000,0.05,0.26)
	time_state1(600,seed_list1,10000,0.3,1,0.05,0.26)


	plt.xlabel("Time")
	plt.ylabel("Proportion of individuals(%)",)

	#plt.title('Without selective exposure',fontsize=12)  
	#plt.title('Pu=0.3',fontsize=12)  
	plt.legend(loc='upper right')
	plt.tight_layout()
	plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG1a.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
