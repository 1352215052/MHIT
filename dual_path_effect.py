...
def seed_create(seed_num,s_start,e):
	...
	#第二步：随机选择初始传播节点
	s_start1=int(s_start*seed_num)  # s传播者初始比例
	e_start=int(e*seed_num) # e免疫者初始比例


	s_start_list = random.sample(range(len(seed_list)),s_start1)  #从列表seed_list中随机选择s_start个元素的索引，注意是索引！
...
def state_attenuation(seed_list_convert,s_start,e):
	...
	s_num=int(int(len(s_li))*((0.3)/(s_start+math.exp(1-ps))))  

	s_indices = random.sample(range(len(s_li)),s_num)
	for i in s_indices:
		s_li[i]=random.randint(2,8) 




	e_num=int(int(len(e_li))*((0.3)/(e+math.exp(1-pe))))  #表示最大的时候衰减率为30%

	e_indices = random.sample(range(len(e_li)),e_num)
	for i in e_indices:
		e_li[i]=random.randint(-2,3) 
...
#1、绘图函数(时间--各群体状态变化曲线）)
def time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e):
	s=[]  #用来存储传播者的人数占比变化
	w=[]  #用来存储无知者的人数占比变化
	e_list1=[]
	#控制时间（时步）
	for i in range(0,t_control):
		...
		if i ==0:  #第一次（初始值）不参与状态转换
			...
			seed_list_attenuation=state_attenuation(seed_list,s_start,e)  #衰减函数处理
...
#稳定性检验函数
def stability_test(t_test,t_control,seed_num,e,num_prop,fc_prop,s_start):
	e_last_list=[]
	for i in range(0,t_test):
		seed_list=seed_create(seed_num,s_start,e)
		e_last=time_state(t_control,seed_list,seed_num,num_prop,fc_prop,s_start,e)
		e_last_list.append(e_last)
...
#1、绘图函数(时间--各群体状态变化曲线）)
def threshold_find(start_thred,end_thred,t_test,t_control,seed_num,num_prop,fc_prop,s_start):
	e_avg=[] #平均值列表
	e=start_thred  #初始阈值
	num=(end_thred-start_thred)*100
	#for i in range(int(num)+1):
	for i in range(int(num)):
		e_last_list=stability_test(t_test,t_control,seed_num,e,num_prop,fc_prop,s_start)
		average=sum(e_last_list)/len(e_last_list)
		e_avg.append(average)
		e+=0.01



	fc=round((fc_prop*0.1),2)
	dt=0.01  #可以不为整数或者1
	t= np.arange(start_thred, end_thred, dt)  # 生成时间数组  



	if unnorl==1:
		  
		plt.plot(t,e_avg,label='Pu={}'.format(fc),color="#0072B2", linestyle="-")
	elif unnorl==2:
		plt.plot(t,e_avg,label='Pu={}'.format(fc),color="#D55E00",  linestyle="-.")
	else:
		plt.plot(t,e_avg,label='Pu={}'.format(fc),color="#009E73",  linestyle="--")
...
unorl_list=[1,2,3]
for unnorl in unorl_list:
	threshold_find(0.05,0.15,10,200,10000,0.3,unnorl,0.05)

 
plt.xlabel('Initial immune proportion(%)')  
plt.ylabel('Equilibrium immune proportion(%)')  
plt.legend(loc='upper left')  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG17a.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
