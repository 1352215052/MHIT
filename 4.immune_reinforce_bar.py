...
def state_bar(seed_list_find):
	...
	a=1
	# # 绘制直方图  
	# plt.figure(figsize=(3,2))  # 设置图形大小（宽，高），单位为英寸
	# # 设置全局字体大小和DPI
	...
	for seed_list in seed_list_find:
		if a==1:
			plt.hist(seed_list, label='Believer',color='#D55E00', alpha=0.7, bins=10)  
			# plt.axvline(x=0, color=(1,0,0), linestyle='-', linewidth=1.5, label='split line')
			# plt.axvline(x=4, color=(1,0,0), linestyle='-', linewidth=1.5)
		elif a==2:
			plt.hist(seed_list, label='Neutral',color="#009E73", alpha=0.7, bins=2)  
		else:
			plt.hist(seed_list, label='Immune',color='#0072B2', alpha=0.7, bins=5)  
...
#plt.label= "attitude_devi={}".format(por_val)
#plt.text("attitude_devi={}".format(por_val))  # (x, y) 位置
#attitude_devi=avg_e-avg_s

# plt.axvline(x=avg_s, color=(1,0,0), linestyle='-', linewidth=1.5)
# plt.axvline(x=avg_e, color=(1,0,0), linestyle='-', linewidth=1.5)
#水平辅助线axhline()
plt.xlabel('Cognitive antibody level(CAL)')  
plt.ylabel('Number of nodes') 

plt.legend()  
plt.savefig(r'E:\AAA\论文写作\A群体免疫\A优化模型\写作内容\投稿\英文投稿\英文\CHB\退修\imag\FIG6b.tif',
			 format="tif" ,
			 dpi=1000,
			 bbox_inches="tight",
...
plt.show()  #显示图形
# for i in range(5):
# 	seed_list=seed_create(1000,0.05,0.7)
# 	time_state(600,seed_list,1000,0.15,1,0.05,0.7)
